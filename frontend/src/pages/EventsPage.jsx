      ...
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
          ...
          <h1 className="font-display text-5xl md:text-6xl text-cream font-300">
            Upcoming <span className="italic text-moxie-400">Events</span>
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-8 py-12">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-10">
          <div className="flex flex-wrap gap-2">
            {CATS.map(c => (
              <button key={c} onClick={() => setFilter(c)} className={`px-4 py-2 text-xs uppercase tracking-wider font-sans border transition-all duration-200 ${filter === c ? 'bg-moxie-400 border-moxie-400 text-white' : 'border-white/10 text-cream/50 hover:border-white/30 hover:text-cream'}`}>
                {c}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            {['grid', 'list'].map(v => (
              <button key={v} onClick={() => setView(v)} className={`px-4 py-2 text-xs uppercase tracking-wider font-sans border transition-all ${view === v ? 'bg-moxie-400 border-moxie-400 text-white' : 'border-white/10 text-cream/50 hover:text-cream hover:border-white/30'}`}>
                {v}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((ev, i) => (
              <motion.div key={ev.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className="group border border-white/5 hover:border-moxie-400/30 bg-stone-900/40 transition-colors duration-300 overflow-hidden">
                ...
                <div className="p-5">
                  <h3 className="font-display text-xl text-cream font-400 mb-1 group-hover:text-moxie-300 transition-colors">{ev.name}</h3>
          ...
          <div className="space-y-3">
            {filtered.map((ev, i) => (
              <motion.div key={ev.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                className="flex items-center gap-6 p-5 border border-white/5 hover:border-moxie-400/20 bg-stone-900/30 group transition-colors">
                <div className="w-16 h-16 bg-moxie-400/10 border border-moxie-400/20 flex flex-col items-center justify-center flex-shrink-0">
                  <div className="font-display text-2xl text-moxie-400 leading-none">{new Date(ev.event_date).getDate()}</div>
                  <div className="text-cream/40 text-xs uppercase">{new Date(ev.event_date).toLocaleString('default', { month: 'short' })}</div>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-sans text-cream font-500 group-hover:text-moxie-300 transition-colors truncate">{ev.name}</h3>
